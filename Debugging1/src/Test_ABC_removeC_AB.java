/**
 * Tests for Change Scenario: [A,B,C] -> remove(C) -> [A,B]
 * 
 * @author Matt T, mvail
*/
public class Test_ABC_removeC_AB
{
	// Label for tests
	private final String label = "SCENARIO: [A,B,C] -> remove(C) -> [A, B]";

	/**
	 * Returns a SimpleSet with two elements A and B. 
	 *
	 * @return - set that contains [A, B] after remove(C)
	 */
	private SimpleSet<Character> newSet()
	{
		...
		// run scenario
		set.remove(SetTests.ELEMENT_C);
		return set;
	}

	/** 
	 * Run tests on scenario: [A,B,C] -> remove(C) -> [A,B]
	 */
	public void run()
	{
