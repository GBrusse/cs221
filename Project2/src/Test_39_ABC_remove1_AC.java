...
public class Test_39_ABC_remove1_AC
{
	...
	public void run(ListType listType)
	{
		...
		try 
		{
			...
			initialize(); 
			result = IndexedUnsortedListTests.listIterator(list, FIRST_INDEX, Result.True); 
			RunIndexedUnsortedTests.printResults("listIterator(FIRST_INDEX)", result);

			// Test: listIterator(int)
			initialize(); 
			result = IndexedUnsortedListTests.listIterator(list, LAST_INDEX, Result.True); 
			RunIndexedUnsortedTests.printResults("listIterator(LAST_INDEX)", result);

			// Test: listIterator(int)
			initialize(); 
			result = IndexedUnsortedListTests.listIterator(list, NEG_INVALID_INDEX, Result.IndexOutOfBounds); 
			RunIndexedUnsortedTests.printResults("listIterator(NEG_INVALID_INDEX)", result);

			// Test: listIterator(int)
			initialize(); 
			result = IndexedUnsortedListTests.listIterator(list, POS_INVALID_INDEX, Result.IndexOutOfBounds); 
			RunIndexedUnsortedTests.printResults("listIterator(POS_INVALID_INDEX)", result);
